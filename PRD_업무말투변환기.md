...
- [x] 프론트엔드 'index.html' 페이지가 FastAPI 서버에서 요청 되도록 정적라우팅(static routing) 기능이 있어야 한다
...
- [x] Vercel에서 프론트엔드가 정상 접속된다
- [x] 배포된 URL에서 실제 변환이 작동한다
- [x] 백엔드와 프론트엔드가 모두 vercel에 같이 배포(deploy) 되도록 한다
...
# 패키지 설치
pip install fastapi==0.136.1 uvicorn==0.47.0 langchain==1.3.1 langchain-upstage==0.7.7 python-dotenv==1.2.2 pydantic==2.13.4
...
5. [x] `requirements.txt` 라이브러리의 버전을 명시하고 최신 버전을 사용한다
6. [x] `requirements.txt` 작성 및 가상환경 폴더를 생성한 후에 패키지 설치
...
4. playwright MCP를 사용한 브라우저 테스트
...
1. [x] `vercel.json` 파일 작성 (백엔드 Serverless Function + 프론트엔드 Static 설정)
2. [x] GitHub 레포지토리에 전체 코드 푸시
3. [ ] Vercel 프로젝트 생성 및 GitHub 레포지토리 연결
4. [ ] Vercel 설정에서 `UPSTAGE_API_KEY` 환경 변수 추가
5. [ ] 배포 후 실제 URL에서 동작 확인 (API와 프론트엔드가 동일 도메인에서 작동)
...
### vercel.json (Combined Deployment)
...
```json
{
  ...
  "rewrites": [
    ...
    { "source": "/health", "destination": "backend/main.py" },
    { "source": "/(js|css)/(.*)", "destination": "/frontend/$1/$2" },
